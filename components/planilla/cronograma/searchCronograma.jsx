...
const SearchCronograma = ({ cronograma, isClose = null, onSelect = null, show = false }) => {
    ...
    const [mes, setMes] = useState(cronograma.month || fecha.getMonth() + 1);
    ...
    const getCronogramas = async (add = false) => {
        ...
        params.set('principal', cronograma?.planilla?.principal);
        await microPlanilla.get(`cronogramas`, { params })
        .then(res => {
            let { meta, items } = res.data;
                                ...
                                <List.Item key={`list-people-${indexC}`}>
                                    <List.Content floated='right'>
                                        <Button color="black"
                                            ...
                                            disabled={(current_loading) || (cronograma.id == cro.id)}
                                        >
                                            Ir
                                        </Button>
                                    </List.Content>
                                    <List.Content>
                                        <b>Planilla [{cro?.planilla?.name}]</b> 
                                        <Show condicion={cro.adicional}>
                                            <span className="badge badge-primary ml-2">Adicional {cro.adicional}</span> 
                                        </Show>
                                        <span className="badge badge-warning ml-2"><i className="fas fa-user"></i> {cro.historialsCount}</span>
                                    </List.Content>
                                </List.Item>    
                            )}
                            {/* preloader */}
