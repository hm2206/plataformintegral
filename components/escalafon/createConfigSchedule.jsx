...
const CreateClock = ({ handleAdd }) => {
    ...
    const handleSave = async () => {
        ...
        await configScheduleProvider.store(form, options)
        .then(res => {
            let { config_schedule, message } = res.data;
            Swal.fire({ icon: 'success', text: message });
            setErrors({});
            setForm({});
            handleAdd(config_schedule);
        }).catch(err => {
    ...
    return (
        <div className="card">
            <div className="card-header">
                <i className="fas fa-plus"></i> Crear Configuración de Horario
            </div>
