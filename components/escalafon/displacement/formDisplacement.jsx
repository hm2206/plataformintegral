...
const FormDisplacement = ({ children, form = {}, errors = {}, className = null, readOnly = [], onChange = null, disabled = false }) => {
    ...
    return (
        <Form className={className}>
            <div className="row">
                ...
                <Form.Field className="col-md-6 mb-3" error={errors?.dependencia_id?.[0] ? true : false}>
                    <label htmlFor="">Dependencia <b className="text-red">*</b></label>
                    <SelectDependencia
                        name="dependencia_id"
                        disabled={readOnly.includes('dependencia_id') || disabled}
                ...
                <Form.Field className="col-md-6 mb-3" error={errors?.perfil_laboral_id?.[0] ? true : false}>
                    <label htmlFor="">Perfil Laboral <b className="text-red">*</b></label>
                    <SelectDependenciaPerfilLaboral
                        name="perfil_laboral_id"
                        disabled={readOnly.includes('perfil_laboral_id') || disabled}
                        value={form?.perfil_laboral_id || ""}
                        onChange={(e, obj) => handleChange(e,  obj)}
                        dependencia_id={form?.dependencia_id}
                    />
                    <label htmlFor="">{errors?.perfil_laboral_id?.[0] || ""}</label>
                </Form.Field>
...
export default FormDisplacement;