...
const Ascenso = ({ work }) => {
    ...
    const getDatos = async (add = false) => {
        ...
        await escalafon.get(`work/${work.id}/vacacion?page=${current_page}`)
            .then(res => {
                let { success, message, vacaciones } = res.data;
                if (!success) throw new Error(message);
                setError(false);
                setCurrentTotal(vacaciones.total);
                setCurrentLastPage(vacaciones.last_page);
                setCurrentData(add ? [...current_data, ...vacaciones.data] : vacaciones.data);
            })
                    ...
                    <div className="card-body">
                        <div className="row">
                            <div className="col-md-10">
                                ...
                                <div className="mb-2"><b>Fecha Límite: </b> {moment(d.fecha_limite).format('DD/MM/YYYY')}</div>
                                <div className="mb-2"><b>Días Ejecutados: </b> {d.dias_ejecutados}</div>
