...
class PermissionProvider extends BaseProvider  {
    ...
    store = async (body = {}, config = {}, ctx = null) => {
        return await authentication.post(`${this.collection}`, body, config, ctx)
            .then(res => res.data)
            .catch(err => this.handleError(err));
    }
    ...
    update = async (id, body = {}, config = {}, ctx = null) => {
        return await authentication.post(`${this.collection}/${id}?_method=PUT`, body, config, ctx)
            .then(res => res.data)
            .catch(err => this.handleError(err));
    }

    delete = async (id, body = {}, config = {}, ctx = null) => {
        return await authentication.post(`${this.collection}/${id}?_method=DELETE`, body, config, ctx)
            .then(res => res.data)
            .catch(err => this.handleError(err));
    }
